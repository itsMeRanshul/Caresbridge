...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/users/${contextUserId}`); // Use contextUserId only
        if (!response.ok) {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/users/${contextUserId}`, {
        method: "PUT",
        headers: {
